...
```bash
ros2 action send_goal /hint_behavior_server/execute_behavior_tree btcpp_ros2_interfaces/action/ExecuteTree "$(jq -n --arg tree Mission --arg xml '<root BTCPP_format="4"><BehaviorTree ID="Mission"><SubTree ID="RunMission" mission_path="/root/turtlebot3_ws/src/hint_narrative/missions/bedroom_to_living_room/mission.txt"/></BehaviorTree></root>' '{target_tree: $tree, payload: $xml}')" --feedback
```
