...
|---|---|---|
| `PlanTrajectoryAction` | `include/hint_behavior/nodes/plan_trajectory_action.hpp` | `/trajectory_generator/plan_trajectory` |
| `FollowTrajectoryAction` | `include/hint_behavior/nodes/follow_trajectory_action.hpp` | `/trajectory_navigator_node/follow_trajectory` |
...
|---|---|---|
| `follow_planned_trajectory.xml` | `FollowPlannedTrajectory` | Plan → follow → turn. `PlanTrajectoryAction` grounds a text `description` + the `{images}` frame buffer into ordered ground `markers` **and** a `turn_degrees` (end-of-move in-place rotation); `FollowTrajectoryAction` drives the path (via `hint_navigation`'s `trajectory_navigator` → Nav2 `follow_path`/MPPI); `SpinAction` then rotates by `turn_degrees` (0° = instant success, so any turn value succeeds; only the physical spin failing fails). Empty `markers` = a turn-only "scan / re-orient" move — the follow is a no-op and only the spin runs. Surfaces `{plan_message}` (the planner's VLM reasoning) for the caller. `description` and `images` are unbound (`{description}`, `{images}`) so it's driven as a `SubTree` — `images` is the unified `hint_narrative` buffer the planner plans over (no camera in the planner). |
| `run_mission.xml` | `RunMission` | Drive a mission to completion: a `KeepRunningUntilFailure` loop of `MissionAdvance` → `FollowPlannedTrajectory` → `SetBlackboard`. `MissionAdvance` is the whole `hint_narrative` interface — each tick reports the last move's outcome (`success` + the planner's VLM reasoning `{plan_message}` as `observation`) and returns the next directive **plus this cycle's `{images}` frame buffer**, which `FollowPlannedTrajectory` forwards to `PlanTrajectoryAction` (one buffer shared by the director and the planner); the `SetBlackboard` pair captures follow success into `{last_ok}`. The leaf also sends `first` (a per-run latch: true on its first tick of the run, false after — the leaf instance is rebuilt per `ExecuteTree` goal) so `narrative_navigation` wipes + reseeds exactly once per run, an explicit signal rather than one inferred from an empty observation. **Which mission runs is chosen at the call** via the `{mission}` port (a YAML path) — no default; bind it in the payload to run/switch missions without restart. `narrative_navigation` folds each outcome into its rolling narrative (one `visual_reasoner` call) and decides the next instruction + completion, so the tree just loops report → execute → capture. Divergence is absorbed (no abort); the one failure path is the stuck backstop, so the outer `Precondition` maps a clean finish to `SUCCESS` and `{mission_failed}` to `FAILURE`. Requires `narrative_navigation` and `visual_reasoner` running. |
...
`RunMission` drives a mission to completion. Requires `narrative_navigation` and `visual_reasoner` (and the `trajectory_generator` / `trajectory_navigator` + Nav2 pipeline) running. **No default mission** — choose it at the call by binding `mission` on a one-line wrapper `SubTree` into the preloaded `RunMission` (the node loads it, resuming that mission's narrative if it exists; no restart):
