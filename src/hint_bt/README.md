...
| `MissionAdvance` | `include/hint_bt/nodes/mission_advance_action.hpp` | `/mission_planner_node/advance` |
...
pipeline) running. **There is no default mission** — you choose it at the call by binding `mission`
on a one-line wrapper `SubTree` into the preloaded `RunMission` (the node loads it, resuming that
mission's narrative if it exists; no node restart):
...
(An empty `payload: ''` runs `RunMission` with no mission bound — it only works if you preloaded one
via the node's `mission_path` param, otherwise the first `advance` fails cleanly.)
