...
class LKTrackerNode(Node):
    def __init__(self):
        ...
        self.declare_parameter(
            "fb_thresh", 2.0
        )  # forward-backward round-trip error threshold (pixels)
        self.declare_parameter(
            "ema_alpha", 0.20
        )  # corner smoothing: lower = smoother, more lag
        self.declare_parameter(
            "ncc_thresh", 0.60
        )  # appearance similarity floor; below this = occluded/distractor
        self.declare_parameter(
            "ncc_redetect_thresh", 0.75
        )  # must exceed this to allow feature re-detection
        self.declare_parameter(
            "inlier_ratio_thresh", 0.60
        )  # RANSAC inlier fraction; below this = two motions present
        ...
        self.create_subscription(
            RegionOfInterest, "/tracking/input_bbox", self.bbox_callback, 10
        )
        ...
        self.get_logger().info(
            "LK tracker ready — waiting for input bbox on /tracking/input_bbox."
        )
        ...
        if not self.initialized:
            ...
            if pts is None or len(pts) < min_features:
                self.get_logger().warn(
                    "Too few features in received bbox — send another."
                )
                cv2.putText(
                    frame,
                    "TOO FEW FEATURES — send new bbox",
                    (50, 50),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.7,
                    (0, 0, 255),
